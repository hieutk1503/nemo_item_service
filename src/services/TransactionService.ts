...
class TransactionService {
    ...
    async purchaseItem(gameId: string, productId: number, userData: UserData) {
        ...
        try {
            // 1. LẤY THÔNG TIN SẢN PHẨM
            // Kiểm tra xem sản phẩm có tồn tại và thuộc game_id này không
            productData = await storeManager.findItemById(productId, gameId);
            if (!productData) {
                return ServiceResponse.Fail("Sản phẩm không tồn tại hoặc không thuộc trò chơi này!");
            }

            // 2. LẤY THÔNG TIN GAME (Lấy baseUrl và code để chuẩn bị gọi thanh toán)
            const gameInfo = await prisma.game.findFirst({
                where: { 
                    OR: [
                        { id: isNaN(Number(gameId)) ? undefined : Number(gameId) },
            ...
            // 3. GỌI API THANH TOÁN BÊN THỨ BA (Subscription Init)
            // Lưu ý: Đoạn này đang để Mock (giả lập) ở dưới, khi nào dùng thật thì mở đoạn try-catch này ra
            // try {
            //     const paymentUrl = `${gameInfo.baseUrl}/api/${gameType}/subscription/init`; 

            //     // Cấu hình axios chặt chẽ để tránh lỗi "Maximum redirects exceeded"
            //     const paymentResponse = await axios.post(paymentUrl, {
            //         msisdn: userId,
            //         productId: productId,
            //         amount: productData.price,
            //         description: `User ${userId} mua ${productData.item_name} trong game ${gameId}`
            //     }, {
            //         maxRedirects: 0, // ✅ Chặn redirect tự động để tránh vòng lặp chết
            //         timeout: 5000    // Timeout 5s để tránh treo server
            //     });
            ...
            //     console.log(`[PAYMENT] Thanh toán thực tế thành công cho user ${userId}`);

            // } catch (payError: any) {
            //     // 🛠 LOGIC TEST: Nếu bạn chưa có API thật, hãy tạm thời bỏ qua lỗi này 
            //     // hoặc comment khối try-catch thanh toán này lại để test cộng đồ.
            //     OrderActionLogger.error('PAYMENT_FAILED', { userId, gameType, error: payError.message });
            //     return ServiceResponse.Fail(`Thanh toán thất bại: ${payError.message}`);
            // }
            console.log(`[TEST MODE] Đã bỏ qua thanh toán thực tế, chấp nhận giao dịch cho user ${userId}`);

            // 4. CỘNG VẬT PHẨM (Chỉ chạy khi bước 3 đã thành công)
            try {
                // Thêm log để check đầu vào trước khi gọi ItemService
                console.log(`[DEBUG] Bắt đầu trao đồ: User=${userId}, Game=${gameId}, ItemID=${productId}`);

                const grantResult = await ItemService.grantItem(
                    userId,             
                    gameId,       
                    Number(productId), // ✅ Đảm bảo là Number
                    1,                                  
                    'STORE_PURCHASE'    
                );

                console.log(`[DEBUG] Trao đồ thành công: InventoryID=${grantResult.inventory_id}`);

            } catch (invError: any) {
                // ✅ QUAN TRỌNG: In lỗi chi tiết ra Terminal để biết tại sao fail
                console.error("❌ LỖI TRAO ĐỒ CHI TIẾT:", invError.message);
                ...
                return ServiceResponse.Fail(`Đã trừ tiền nhưng không thể nhận đồ. Lỗi: ${invError.message}`);
            }
            // 5. LƯU ĐƠN HÀNG THÀNH CÔNG VÀO MYSQL
            const orderRes = await OrderService.createOrder({
                gameId: gameId,
